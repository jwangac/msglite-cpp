...
// pedantic checks
static_assert(sizeof(bool) == 1, "bool must be one byte");
...
namespace {
    ...
    // Compilers like gcc and clang can recognize these endian-conversion
    // functions and optimize them to one instruction.
    template <typename T>
    void to_1_bytes(T y, Slice x)
    {
    ...
    template <typename T>
    void from_1_bytes(T& y, ReadonlySlice x)
    {
        static_assert(sizeof(T) == 1, "Data type size must be 1 bytes");
        union {
            T t;
    ...
    template <typename T>
    void from_2_bytes(T& y, ReadonlySlice x)
    {
        static_assert(sizeof(T) == 2, "Data type size must be 2 bytes");
        union {
            T t;
    ...
    template <typename T>
    void from_4_bytes(T& y, ReadonlySlice x)
    {
        static_assert(sizeof(T) == 4, "Data type size must be 4 bytes");
        union {
            T t;
    ...
    template <typename T>
    void from_8_bytes(T& y, ReadonlySlice x)
    {
        static_assert(sizeof(T) == 8, "Data type size must be 8 bytes");
        union {
            T t;
    ...
    int custom_strnlen(const char* str, size_t n)
    {
    ...
    uint32_t crc32b(uint32_t crc, ReadonlySlice buf)
    {
        crc = crc ^ ~0U;
    ...
    // See specification of MessagePack.
    // https://github.com/msgpack/msgpack/blob/master/spec.md
    int8_t bytes_of_type(uint8_t type_byte)
    {
        switch (type_byte) {
            // Bool (false)
            case 0xC2:
...
bool Object::cast_to(char* x) const // Assumes sizeof(x) >= 16
{
    if (type == String) {
        int len = custom_strnlen(as.String, 16);
        if (len > 15)
            return false;
        strncpy(x, as.String, 16);
        return true;
    }
    return false;
}
...
int16_t MsgLite::Pack(const Message& msg, uint8_t* _raw_buf, uint8_t _len)
{
    Slice buf = Slice(_raw_buf, _len);

    int16_t msg_size = msg.size();
    if (msg_size < 0 || msg_size > buf.len)
        return -1; // invalid message or buffer size is insufficient
...
bool MsgLite::Unpack(const uint8_t* _raw_buf, uint8_t _len, Message& msg)
{
    ReadonlySlice buf = ReadonlySlice(_raw_buf, _len);
